import { View, Text, Image, TouchableOpacity, ScrollView, StyleProp, ViewStyle } from 'react-native'
...
const StarRating = ({ rating, styles }: { rating?: number, styles: StyleProp<ViewStyle> }) => {
  return (
    <AirbnbRating
      count={5}
      starImage={star}
...
const ReqTripScreen = ({ route }: { route: RouteProp<any> }) => {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} >
      ...
      <View className={`border-b border-b-[#FFA800] h-min mx-4 py-4 gap-y-1`}>
        ...
        <View className={` pl-2 gap-y-[2px] max-w-[70%]`}>
          ...
          <View className={`flex-row items-start ml-[-11px]`}>
            ...
            <Text className={`ml-1 text-base`}>{`${location.city}, ${location.state}`}</Text>
          </View>
        </View>
      ...
      <ButtonReqTravel requestSent={requestSent} requestAccepted={requestAccepted} setRequestSent={setRequestSent}
      />
    </ScrollView>
  )
}
